...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    // check if tlb knows the value of base_virtual_addr
    ...
    if(pte_base == NULL){
        ...
        if(dir_entry->present == 0){
            // present flag clear, page fault. No page directory
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    // check seg fault
    addr32 phy_addr_base = pte_base->base_addr;
    if(address_in_segmentations(this->segmentations[pid], virtual_address) == 0 || pte_base->read_write == 0){
        mmu_raise_segmentation_fault(this);
        return;
    }
    pte_base->dirty = 1;
