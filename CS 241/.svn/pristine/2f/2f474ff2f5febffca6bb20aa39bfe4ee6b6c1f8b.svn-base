...
uint32_t syscall_mmap(uint32_t length, int prot_flags, int flags,
    ...
    if ((flags & MAP_ANONYMOUS) == 0 && offset % PAGE_SIZE) {
	return MAP_FAILED;
    }
    ...
    list_rev_for_each(current->stack_segment, current->heap_segment, area_prev, area_next, elem) {
/*	cur = elem;
	if (prev != NULL) {
	    if (prev->start_addr - elem->end_addr >= length) {
		start = elem->end_addr;
		break;
	    }
	}
	prev = elem;
*/
	prev = list_prev(elem, area_prev, area_next);
	if (prev == NULL) return MAP_FAILED;
	if (elem->start_addr - prev->end_addr >= length) {
	    end = elem->start_addr;
	    break;
	}
    }
    ...
    struct vm_area *newvma = create_vm_area(current, end-length, end);
    if (prot_flags & PROT_READ) newvma->prot_read = 1;
    if (prot_flags & PROT_WRITE) newvma->prot_write = 1;
    if (prot_flags & PROT_EXEC) newvma->prot_exec = 1;
    if (flags & MAP_PRIVATE) newvma->map_private = 1;
    if (flags & MAP_ANONYMOUS) newvma->map_anonymous = 1;
    if (pathname && (flags & MAP_ANONYMOUS) == 0) {
	struct mapped_file *mf = obtain_mapped_file(pathname);
	if (mf == NULL) return MAP_FAILED;
	mf->reference_count++;
	newvma->file = mf;
	newvma->page_offset = offset;
	if ((flags & MAP_SHARED) != 0) {
	    list_link_before(newvma, &mf->shared_vma_list, area_prev, area_next);
	}
    }
    sorted_link_vm_area(current, newvma);
    return newvma->start_addr;
}
...
addr32 syscall_sbrk(uint32_t shift) {
    ...
    struct vm_area *heap = current->heap_segment;
    heap->end_addr += shift;
    return heap->end_addr;
}
/**
 * If `addr` corresponds to a shared page backed by a memory-mapped file,
 * that page will be synced (written back) to disk if dirty (see disk.h).
