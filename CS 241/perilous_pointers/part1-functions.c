...
//    switch (s) {
   char str1[] = "blue";
   char str2[] = "orange";
   if ( strncmp(s, str1, 4) == 0){ 
        printf("Orange and BLUE!\n");
        return;
      }
   else if( strncmp(s, str2, 6) == 0){
        printf("ORANGE and blue!\n");
        return;
      }
   else {
...
int little_automaton(int (*transition)(int, char), const char *input_string) {
    ...
    // put something here
    while(*input_string){
	state = transition(state,(char)* input_string);
        input_string++;
    }
    return state;
}
