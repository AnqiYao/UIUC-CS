...
class KDTree
{
  ...
  private:
     ...
     */
    void helpBuild(vector<Point<Dim>> & subpoint,int min, int max, int d) const;
    void selectionSort(vector<Point<Dim>> & subpoint, int min, int max,int d) const;
    void helpFind(const Point<Dim>& subpoint,Point<Dim>& curBest, int min, int max,int d,bool checkLr) const;
