...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for(size_t i = 0; i< frequencies.size(); i++){
     ...
     TreeNode* final = new TreeNode(small1->freq.getFrequency() + small2->freq.getFrequency());
        final->left = small1;
        final->right = small2;
        mergeQueue.push(final);
       }
      }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
       ...
       if(!bfile.getNextBit()) current = current-> left;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    if(bfile.hasBits()){
         if(bfile.getNextBit()==1){
               TreeNode* next = new TreeNode(Frequency(bfile.getNextByte(),0));
               return next;}

         else {
               TreeNode* next = new TreeNode(0);
               next -> left = readTree(bfile);
               next -> right = readTree(bfile);
               return next;
               }
     }
      return NULL;              
