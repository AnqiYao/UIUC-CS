...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         */
      string temp =  ll.url;
      string temp1 = ll.customer.substr(1,ll.customer.size()-3);
      temp1+=temp;
      if(whenVisitedTable.keyExists(temp1))
      {     if(whenVisitedTable[temp1] < ll.date)
               whenVisitedTable[temp1] = ll.date;
         }
       else 
              whenVisitedTable.insert(temp1,ll.date);
        if(!pageVisitedTable.keyExists(temp))
...
{
    string temp = url;
    string temp1 = customer.substr(1,customer.size()-3);
    temp1+=url;
    return whenVisitedTable.keyExists(temp1);
}
                                  ...
                                  const string& url) const
{
    ...
    string temp1 = customer.substr(1,customer.size()-3);
    temp1 += temp;
    return whenVisitedTable.find(temp1);
