...
size_t find_idx(const std::vector<T>& elements, const C& val,size_t min,size_t max){
    ...
    if (val > elements[mid] && val < elements[mid+1])
        return mid+1;
    if (val < elements[mid])
        return find_idx(elements,val,min,mid-1);
    else 
        return find_idx(elements,val,mid+1,max);  
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
     ...
     return find_idx(elements,val,0,elements.size()-1);            
