...
class BinaryTree
{
  ...
  private:
    ...
    const Node* isOrdered1(const Node* subRoot) const;

    const Node* isOrdered2(const Node* subRoot) const ;
    
    void printPaths(Node* subRoot,int path[], int index) const;
 
    int totalHeight(Node* subRoot,Node* current, int sum, int num) const;

    int pathDistance(Node* subRoot, Node* target,int index) const;
