...
{     
    ...
    std::transform(newword1.begin(), newword1.end(), newword1.begin(), ::toupper);
    std::transform(newword2.begin(), newword2.end(), newword2.begin(), ::toupper);
    auto lookup1 = dict.find(newword1);
    auto lookup2 = dict.find(newword2);
    if(lookup1==dict.end() || lookup2 == dict.end() || lookup1->second != lookup2->second)
        return false;
    /* Your code goes here! */
    return true;
}
